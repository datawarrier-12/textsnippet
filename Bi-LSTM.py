...
data = pd.read_csv('fake_or_real_news1.csv')
data = data.loc[:, ['title', 'text']]

#data.drop_duplicates('Text', inplace = True)     # drop duplicate reviews
data.drop_duplicates('title', inplace = True)     # drop duplicate reviews
...
def text_cleaner(text):
       ...
       def expand_contractions(s, contractions_dict=contraction_mapping):
           def replace(match):
               return contractions_dict[match.group(0)]

           return contractions_re.sub(replace, s)

       cleaned = sent_tokenize(text)
       cleaned = [expand_contractions(i) for i in cleaned]
       cleaned = [re.sub('\n', '', i) for i in cleaned]
       ...
       tokens = [w for w in cleaned if w not in stop_words]
...
cleaned_text = [text_cleaner(text) for text in data['title']]
...
cleaned_summary = [summary_cleaner(text) for text in data['text']]
...
train_y = pad_sequences(train_y, maxlen = max_len_text, padding = 'post')
...
## LSTM 1
encoder_lstm = layers.Bidirectional(layers.LSTM(128, activation= 'relu', dropout=0.2, input_shape =(50,), return_sequences = True, return_state = True))
#encoder_LSTM_rev = layers.LSTM(latent_dim, return_state=True, return_sequences=True, dropout_U=0.05, dropout_W=0.05, go_backwards=True)
#encoder_LSTM_rev = layers.LSTM(128, activation= 'relu', return_state=True, go_backwards=True)
encoder_outputs, state_h, state_c, *_ = encoder_lstm(encoder_emb)
#encoder_outputsR, state_hR, state_cR = encoder_LSTM_rev(encoder_emb)
...
decoder_lstm = layers.LSTM(128, activation= 'relu', dropout=0.2, input_shape =(50,), return_sequences = True, return_state = True)
...
#decoder_outputs2, state_h2, state_c2 = decoder_lstm(128, initial_state = [decoder_state_input_h, decoder_state_input_c])
...
def decode_sequence(input_seq):
       ...
       while not stop_condition:
              ...
              if sampled_token == 'endsum' or len(decoded_sentence.split()) >= max_len_text - 1:
                     stop_condition = True
...
for i in range(0,10):
    ...
    sm = seq2summary(train_y[i])
    print("Original summary:", sm)
    score = sentence_bleu(cleaned_summary, sm)
    print(score)
